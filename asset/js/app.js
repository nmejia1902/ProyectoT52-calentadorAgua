...
window.state = {
 ...
 const LIMITS = { UPPER: 95, LOWER: 10 };
 ...
 function setDisabled(b, d) { b.disabled = !!d; b.style.opacity = d ? 0.6 : 1; }
 ...
 function startBeep() {
   if (!state.alarmSoundOn) return;
   initAudio();
   stopBeep();
   beepTimer = setInterval(() => { beepGain.gain.value = beepGain.gain.value === 0 ? 0.1 : 0; }, 300);
 }
 function stopBeep() {
   if (beepTimer) { clearInterval(beepTimer); beepTimer = null; }
   if (beepGain) beepGain.gain.value = 0;
 }
 function updateUI() {
   ...
   if (state.auto && state.drainOpen) valvesStatus += " (AUTO)";
   ...
   els.waterFill.style.fill = (state.level >= LIMITS.UPPER + 2 || state.level <= LIMITS.LOWER + 5)
     ? "#F59E0B" : "url(#waterGrad)";
   ...
   els.coilGroup.style.opacity = state.heaterOn ? 0.9 : 0.25;
 ...
 function loop() {
   if (state.on) {
     ...
     if (state.auto) {
       ...
       if (state.level <= LIMITS.LOWER && state.drainOpen) state.drainOpen = false;
       if (state.level <= LIMITS.LOWER + 5 && !state.fillOpen && !state.drainOpen) state.fillOpen = true;
       if (state.level >= LIMITS.UPPER + 2 && state.fillOpen) state.fillOpen = false;
     }
     ...
     if (state.heaterOn) state.t += K.heatRate * dt; else state.t -= loss;
 ...
 els.btnFill.onclick = () => { if (!state.on || state.drainOpen) return; state.fillOpen = !state.fillOpen; updateUI(); };
 els.btnDrain.onclick = () => { if (!state.on || state.fillOpen) return; state.drainOpen = !state.drainOpen; updateUI(); };
